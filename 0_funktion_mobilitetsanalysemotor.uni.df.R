...
ma.motor.uni.df <- function(x,klasse_label=c("ikke_angivet"),discogrp=FALSE) {

#til test 
# klynger <- manuelle.klynger.lav
# klasse_label=c("ikke_angivet")
# discogrp=FALSE
...
names(mat.e.result) == colnames.tmp #yes 
...
if(discogrp==TRUE){
		mat.e.result <-  df %>%  left_join(mat.e.result,.)
}
