...
# test <-  t(sapply(allbeskaeft, unlist))


# test <-  as.data.frame(allbeskaeft)


# måske denne her kan bruges:

# test <-  rapply(lst, f=function(x) ifelse(is.na(x),0,x), how="replace" )
...
allbeskaeft <- as.matrix(allbeskaeft)
# is.matrix(allbeskaeft)
# view(allbeskaeft)
 allbeskaeft <-  matrix(allbeskaeft, nrow=nrowtab2xl)
# typeof(allbeskaeft)
# is.matrix(allbeskaeft)
...
# is.matrix(mob.mat)
# view(mob.mat)
# mob.mat <- matrix(unlist(mob.mat), nrow=nrowtab2xl)
rownames(mob.mat)  <- mob.mat[,1]
mob.mat  <- mob.mat[,-1]
#mob.mat <- round(mob.mat/2) #test af relativ risiko
# mob.mat <-  mob.mat + mob.mat2
...
# lav total beskaeftigelse til moneca

view(allbeskaeft)
totalbeskaeft_tmp           <- allbeskaeft[,-1]
totalbeskaeft_tmp           <- allbeskaeft
# view(totalbeskaeft_tmp)
