...
# Trappen 5
trappe.niveaus          <- list("1" = 1, "2" = seq(1:2), "3" = seq(1:3), "4" = seq(1:4), "5" = seq(1:5), "6" = seq(1:6)) 
...
s6                      <- segment.membership(seg, niveau = 1:6)[,2]
...
cs5t6                   <- as.character(s5) == as.character(s6)

cs                      <- cs1t2 + cs2t3 + cs3t4 + cs4t5 +cs5t6
cs                      <- as.factor(cs)
level.group             <- substring(as.character(s6), first = 1, last = 1)
...
pt6  <- gg.jonas(seg, layout=lay, niveau=1:6, edges = edges.default, vertex.fill=cs5t6, edge.color = "grey30", midpoints = FALSE, vertex.size = 4,
                 edge.size = 0.3, border.padding = 1.5, show.text = FALSE, border.text.size = 3, border.text = FALSE)
pt6  <- pt6 + list.scales + ggtitle("6th Level") + annotate("segment",x=Inf,xend=-Inf,y=Inf,yend=Inf,color="black",lwd=1)
