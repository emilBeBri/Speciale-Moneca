...
mat.e.result.bak2 <- mat.e.result
mat.e.result <- mat.e.result.bak2




mat.e.result  <-  e.pheat.dataprep(5,1)
heat.krit.df <-  df %>% filter(membership=="3.34" | membership=="3.35" ) %>%  rename(segkrit=`3: Segment`) %>% select(disco,segkrit)   
# view(heat.krit.df)
...
pheatmap(mat.e.result, color = colorRampPalette(rev(brewer.pal(n = 7, name = "RdYlBu")))(length(breaksList)), breaks = breaksList, cluster_cols=FALSE,cluster_rows=FALSE, display_numbers = pheatmap.disp, fontsize_number=25)
